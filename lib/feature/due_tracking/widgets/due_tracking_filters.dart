...
class DueTrackingFilters extends StatelessWidget {
  const DueTrackingFilters({
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        final statusFilter = Obx(
          () => PanelDropdown<DueRecordDisplayStatusFilter?>(
            label: 'Durum',
            hint: 'Durum',
            compact: true,
            ...
            items: const [
              null,
              DueRecordDisplayStatusFilter.pending,
              DueRecordDisplayStatusFilter.overdue,
              DueRecordDisplayStatusFilter.paid,
            ],
            itemLabel: (value) {
              if (value == null) {
                return 'Tümü';
        ...
        final currencyFilter = Obx(
          () => PanelDropdown<CurrencyType?>(
            label: 'Para birimi',
            hint: 'Para birimi',
            compact: true,
            value: controller.selectedCurrencyFilter.value,
            items: const [
              null,
              CurrencyType.try_,
              CurrencyType.usd,
            ...
            onChanged: (value) {
              controller.selectedCurrencyFilter.value = value;
              controller.searchAndFilterDueRecords();
            },
          ),
        );

        final dueDateRangeFilter = Obx(
          () => _DueDateRangeFilter(
            isCompact: isCompact,
            ...
            children: [
              ...
              Wrap(
                ...
                children: [
                  ...
                  SizedBox(width: _dropdownWidth, child: currencyFilter),
                  clearButton,
                ],
              ),
              const SizedBox(height: AppUiTokens.space8),
...
class _DueDateRangeFilter extends StatelessWidget {
  const _DueDateRangeFilter({
  ...
  Widget build(BuildContext context) {
    ...
    if (isCompact) {
      return Column(
        ...
        children: [
          Text('Vade tarihi', style: labelStyle),
          const SizedBox(height: AppUiTokens.space8),
          Row(
            children: [
