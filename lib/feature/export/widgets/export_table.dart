...
class ExportTable extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: Obx(() {
        final records = controller.exports;
        final isLoading = controller.isLoading.value;
        final isDeleting = controller.isDeleting.value;
        final deletingId = controller.deletingExportId.value;
        ...
        if (records.isEmpty) {
          final message = controller.hasActiveFilters
              ? 'Kriterlere uygun ihracat kaydı bulunamadı.'
              : 'Henüz ihracat kaydı bulunmuyor.';
  ...
  DataRow _buildRow(
    ...
    return DataRow(
      cells: [
        ...
        DataCell(
          Text(
            record.shipmentDate == null
                ? '—'
                : AppDateUtils.formatDate(record.shipmentDate!),
            style: _dataStyle,
          ),
        ),
        DataCell(
          Text(
            record.deliveryDate == null
                ? '—'
                : AppDateUtils.formatDate(record.deliveryDate!),
            style: _dataStyle,
          ),
        ),
        DataCell(
